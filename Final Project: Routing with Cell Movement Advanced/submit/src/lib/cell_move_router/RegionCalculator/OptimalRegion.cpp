...
OptimalRegion::getRegion(const Input::Processed::CellInst *Cell) {
  ...
  int ColBeginIdx = 0;
  int ColEndIdx = 0;
  // int row = InputPtr->getRowSize();
  // int col = InputPtr->getColsize();
  ...
  if (IsChange) {
    ...
    // RowBeginIdx = std::max(RowBeginIdx-1,1);
    // RowEndIdx = std::min(RowEndIdx+1,row);
    // ColBeginIdx = std::max(ColBeginIdx-1,1);
    // ColEndIdx = std::min(ColEndIdx+1,col);
  } else {
